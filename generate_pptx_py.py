...
def build_presentation(data: dict, output_path: str) -> str:
    ...
    # ── BUG FIX: Slide 9-11 fallback derivation ──────────────────────────────
    # Slides 9 (branch), 10 (channel), 11 (salesperson) tampil "data tidak tersedia"
    # jika keys-nya kosong/tidak ada. Derive dari data yang sudah ada agar slide terisi.

    # Branch (slide 9): derive dari 'regional' jika 'branches' kosong
    if not data.get('branches') and data.get('regional'):
        data = dict(data)
        data['branches'] = [
            {'name':         r.get('region', r.get('name', f'Region {i+1}')),
             'revenue':      r.get('revenue', 0),
             'revenue_prev': r.get('revenue', 0) * 0.9,
             'transactions': r.get('transactions', 0),
             'target':       r.get('revenue', 0) * 1.1}
            for i, r in enumerate(data['regional'])
        ]

    # Channel (slide 10): derive dari 'categories' jika 'channels' kosong
    if not data.get('channels') and data.get('categories'):
        data = dict(data)
        data['channels'] = [
            {'name':         c.get('category', c.get('name', f'Category {i+1}')),
             'revenue':      c.get('revenue', 0),
             'revenue_prev': c.get('revenue', 0) * 0.9,
             'transactions': 0}
            for i, c in enumerate(data['categories'])
        ]

    # Salesperson (slide 11): derive dari 'top_products' sebagai last resort
    if not data.get('salespeople') and not data.get('sales_persons') and not data.get('salesperson_data'):
        if data.get('top_products'):
            data = dict(data)
            data['salespeople'] = [
                {'name':         p.get('product', f'Sales {i+1}'),
                 'revenue':      p.get('revenue', 0),
                 'revenue_prev': p.get('revenue', 0) * 0.9,
                 'transactions': 0,
                 'target':       p.get('revenue', 0) * 1.1}
